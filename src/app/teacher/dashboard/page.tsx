...
export default function TeacherDashboard() {
  ...
  function parseStudents(str: string) {
    ...
    const students = studentsArr.map((arr) => {
      const nameArr = arr.slice(1, 4)[0].split(' ');
      const name = nameArr.reduce((acc, cur) => (acc += ruToEnMap[cur[0]].toLowerCase()), '');
      const username = name + arr[2].slice(0, 2) + ruToEnMap[arr[2][2]].toLowerCase() + arr[2].slice(3);
      return {
  ...
  const handleCreateGroup = async () => {
    if (!newGroupList) {
      setSnackbar({
        open: true,
        message: 'Введите данные студентов',
        severity: 'error',
      });
      return;
    }
              ...
              disciplines.map((discipline) => (
                ...
                >
                  <Card
                    ...
                    onClick={() => router.push(`/teacher/discipline/${discipline.id}`)}
                  >
                    <CardContent>
                      <Typography
                        variant="h6"
                ...
                <CardContent>
                  ...
                  {isLoadingGroups ? (
                    ...
                    >
                      ...
                      <Typography variant="body1">У вас пока нет групп.</Typography>
                      <Typography
                        variant="body2"
                        sx={{ mt: 1, mb: 2 }}
                      >
