// app/api/subjects/[subjectId]/details/route.ts (App Router)
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const testsRaw = db
      ...
      .all(subjectId) as { id: number; title: string; chapter_id: number | null }[];
    ...
    // Получаем группы, привязанные к дисциплине
    const groupsRaw = db
      .prepare(
        `SELECT g.id, g.name
       FROM Groups g
    ...
    const discipline = {
      id: subject.id,
      name: subject.name,
      description: '', // Добавьте поле description в таблицу Subjects, если нужно
      groups,
      chapters,
    };
